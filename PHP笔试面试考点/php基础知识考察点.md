...
变量名后+();  
应用变量函数可以回调函数  
###  call_user_func_array()可以回调函数  
1. call_user_func_array('fun',array(1,2))  
2. call_user_func_array(array(new Foo(), 'fun'), array($a,$b))  
3. call_user_func_array((array(Foo, 'fun'), array($a,$b))
### 匿名函数也叫闭包函数  
    $a = function(){};  
    call(function (){});
