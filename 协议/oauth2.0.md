- [1. OAuth 2.0 开放网络授权标准总纲](#zg)
- [2. OAuth 2.0 开放网络授权标准详解](#xj)
- [3. 结语](#jy)
# <a id="zg">OAuth 2.0 开放网络授权标准总纲 </a> 
## 简介  
...
# <a id="xj">OAuth 2.0 开放网络授权标准详解 </a> 
## OAuth2.0 — 授权码模式详解  
...
![client-credentials-flow](https://github.com/MAZENAN/lear_note/blob/master/协议/img/client-credentials-flow.jpg)  
令牌颁发的步骤如下：
...
# <a id="jy">结语</a>  
OAuth 只是一套授权的验证机制，验证用户身份是否合法，是否可以请求后台服务器的接口，用户对数据的访问权限和数据安全是另外一套机制，依旧由咱们进行控制，根据业务需要项目需要设定保护机制（来自coding10）